  ...
  try {
    ...
    const [{ data: profiles }, { data: messages }] = await Promise.all([
      ...
      supabase
        .from("messages")
        .select(
          "id, conversation_id, sender_id, body, media_url, media_type, read_at, created_at"
        )
...
/** Lightweight unread count — does not load full inbox/message bodies. */
...
export async function markConversationRead(
  ...
  try {
    ...
    return { error: e instanceof Error ? e.message : "Could not mark read" };
  }
}

/** Live updates for a thread (INSERT/UPDATE). Never throws into React. */
...
export function subscribeInbox(
  ...
  try {
    ...
    const channel = supabase
      ...
      .subscribe();
    // Slower poll — realtime covers most updates; avoid hammering listInbox.
    const poll = window.setInterval(safeOnChange, 45000);
    return () => {
